  ...
  return (
    <AdminShell title="Reportes" subtitle="Resumen operativo consolidado">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <article className="rounded-xl border border-slate-200 p-3">
          <p className="text-xs uppercase text-slate-500">Depósitos pendientes</p>
      ...
      <section className="mt-5 grid grid-cols-1 xl:grid-cols-2 gap-4">
        <article className="rounded-xl border border-slate-200 p-4">
          <h3 className="text-lg font-bold">Últimos retiros pendientes</h3>
          <ul className="mt-3 space-y-2 text-sm">
            {pendingWithdrawals.slice(0, 6).map((row) => (
              <li key={row.id} className="rounded-lg border border-slate-200 px-3 py-2">
                {row.professional?.firstName} {row.professional?.lastName} · {Number(row.credits).toFixed(2)} cr · {money(Number(row.amountBs ?? row.soles ?? 0))}
              </li>
            ))}
        ...
        <article className="rounded-xl border border-slate-200 p-4">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {history.slice(0, 6).map((row) => (
              <li key={row.id} className="rounded-lg border border-slate-200 px-3 py-2">
                {row.professional?.firstName} {row.professional?.lastName} · {row.status} · {money(Number(row.amountBs ?? row.soles ?? 0))}
              </li>
            ))}
